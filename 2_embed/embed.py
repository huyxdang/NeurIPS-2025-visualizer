...
image = (
    ...
    .pip_install(
        "sentence-transformers>=3.0.0",  # Latest version includes Gemma support
        "torch",
        "pandas",
        "pyarrow", 
        "tqdm",
...
def upload_file_to_volume(file_bytes: bytes, remote_path: str):
...
def download_file_from_volume(remote_path: str) -> bytes:
...
def main(
    input_file: str = "data/neurips_2025_papers_full.parquet",
    output_file: str = "data/neurips_2025_papers_with_embeddings.parquet"
):
    ...
    # Read file and upload to Modal volume
    with open(local_input, "rb") as f:
        file_bytes = f.read()
    
    remote_path = f"/data/{local_input.name}"
    upload_file_to_volume.remote(file_bytes, remote_path)
    ...
    result_path = generator.process_parquet.remote(remote_path)
    
    print(f"✅ Processing complete: {result_path}")
    
    # Download result
    print(f"\n📥 Downloading result to {output_file}...")
    result_bytes = download_file_from_volume.remote(result_path)

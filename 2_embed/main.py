...
Supports both local and Modal (GPU) embedding generation

Workflow:
1. Convert JSON to Parquet
2. Generate embeddings (local CPU/GPU or Modal cloud GPU)
...
class PipelineRunner:
    ...
    def __init__(self, use_modal=False):
        self.base_dir = Path(__file__).parent
        self.use_modal = use_modal
        
        # Choose embedding script based on mode
        embed_script = 'embed.py' if use_modal else 'embed_local.py'
        
        self.scripts = {
            'convert': self.base_dir / 'json_to_parquet.py',
            'embed': self.base_dir / embed_script,
        ...
        self.files = {
            ...
            'parquet': data_dir / 'neurips_2025_papers_full.parquet',
            'embedded': data_dir / 'neurips_2025_papers_with_embeddings.parquet',
            'umap': data_dir / 'neurips_2025_papers_with_umap.parquet',
            'final_json': data_dir / 'neurips_2025_papers_final.json'
        }
    ...
    def run_step(self, script_name, args=None):
        ...
        # For Modal, use 'modal run' instead of 'python'
        if self.use_modal and script_name == 'embed':
    ...
    def step_embed(self, force=False):
        ...
        print(f"STEP 2: Generate Embeddings ({mode})")
        print("="*70)
    ...
    def step_to_json(self, no_embeddings=False):
        ...
        args = [str(self.files['umap']), '-o', str(self.files['final_json'])]
        if no_embeddings:
    ...
    def run_all(self, force=False, to_json=False, no_embeddings=False):
        ...
        print("\n" + "🚀 "*35)
        mode = "Modal GPU" if self.use_modal else "Local"
        print(f"RUNNING COMPLETE PIPELINE ({mode})")
        print("🚀 "*35)
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Run complete pipeline (local, CPU/GPU)
  python pipeline.py --all
  
  # Run with Modal cloud GPU (fast!)
  python pipeline.py --all --modal
  
  # Run specific step
  python pipeline.py --step convert
  python pipeline.py --step embed --modal
...
Pipeline Steps:
  ...
  2. embed   - Generate embeddings (local or Modal)
  3. umap    - Apply UMAP dimensionality reduction
  4. to_json - Convert final Parquet to JSON (optional)

Embedding Options:
  --modal    - Use Modal cloud GPU (30 sec, requires: modal setup)
  (default)  - Use local CPU/GPU (10-20 min)
        """
    )
    ...
    parser.add_argument('--modal', action='store_true',
                       help='Use Modal cloud GPU for embeddings (much faster!)')
    ...
    runner = PipelineRunner(use_modal=args.modal)
